...
*	Write a boolean expression that checks for a given integer if it can be divided (without remainder) by `7` and `5` in the same time.
...
### Problem 7. Is prime 
*	Write an expression that checks if an given positive integer number n (n ≤ 100) is prime.
